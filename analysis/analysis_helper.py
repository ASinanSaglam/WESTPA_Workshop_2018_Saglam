...
def load_mapper(file_name, iter_no):
    ...
    if iter_no:
        it = h['iterations/iter_%08d'%iter_no]
        hasval = it.attrs['binhash']
    else:
        it = h.attrs['west_current_iteration']
        hashval = it.attrs['binhash']
...
def vor_overlay(pylab, hfile, iiter=None):
    mapper = load_mapper(hfile, iiter)
    X = mapper.centers[:,0]
    Y = mapper.centers[:,1]
    pylab.scatter(X,Y)
    segments = vp.voronoi(X,Y)
